...
bool YModem::transmitFileOperate(YModem::Status _status) {
    status = _status;
    uint16_t crc;
    ...
    case StatusEstablish:
        //打开文件
        if (fileOperate(FileOpenReadOnly, nullptr, 0) != OperateFail) {
            auto nameLength = fileOperate(FileGetName, &txBuffer[YMODEM_PACKET_HEAD], 0);
            fileSize  = fileOperate(FileGetSize, &txBuffer[YMODEM_PACKET_HEAD + nameLength + 1], 0);
            fileCount = 0;
    ...
    case StatusTransmit:
        ...
        if (fileSize - fileCount > YMODEM_PACKET_SIZE) {
            fileCount += fileOperate(FileRead, &txBuffer[YMODEM_PACKET_HEAD],
    ...
    default:
        fileOperate(FileClose, nullptr, 0);
        break;
    }
...
void YModem::transmitStageNone() {
    ...
    //    write((uint8_t *)"YModem ready", strlen("YModem ready"));
}
...
void YModem::transmitStageEstablishing() {
    ...
    case CodeC:
        ...
        if (transmitFileOperate(StatusEstablish)) {
    ...
    case CodeCan:
        abort();
        transmitFileOperate(StatusAbort);
        break;
    ...
    default:
        ...
        if ((counter.timeCount / counter.timeInterval) > counter.timeMax) {
            abort();
            transmitFileOperate(StatusTimeout);
        }
        break;
    }
}
...
void YModem::transmitStageEstablished() {
    ...
    case CodeAck:
        ...
        clearCounter();
        stage = StageEstablished;
        if (!transmitFileOperate(StatusTransmit)) {
    ...
    case CodeC:
        ...
        if (counter.errorCount > counter.errorMax) {
            abort();
            transmitFileOperate(StatusError);
        } else {
            clearCounter();
...
void YModem::transmitStageFinished() {
    ...
    case CodeAck:
        abort(false);
        transmitFileOperate(StatusFinish);
        break;
...
bool YModem::receiveFileOperate(YModem::Status _status, uint32_t size) {
    status = _status;
...
//        fileCount = 0;
//        fileOperate(FileSetName, (uint8_t *)fileName.c_str(), fileName.size());
//        fileOperate(FileSetSize, nullptr, fileSize);

        memset(fileName, 0, sizeof(fileName));
        strcpy(fileName, (char *) &rxBuffer[YMODEM_PACKET_HEAD]);
        fileSize  = strtoul((char *) &rxBuffer[YMODEM_PACKET_HEAD + nameLength + 1], nullptr, 10);
        fileCount = 0;
        fileOperate(FileSetName, (uint8_t *) fileName, strlen(fileName));
        fileOperate(FileSetSize, nullptr, fileSize);

        if (fileOperate(FileOpenWriteOnly, &rxBuffer[YMODEM_PACKET_HEAD], nameLength) == OperateFail) {
            // 文件创建失败
            status = StatusError;
            return false;
        }
    ...
    case StatusTransmit:
        ...
        if (fileSize - fileCount > size) {
            fileOperate(FileWrite, &rxBuffer[YMODEM_PACKET_HEAD], size);
            fileCount += size;
        } else {
            fileOperate(FileWrite, &rxBuffer[YMODEM_PACKET_HEAD], fileSize - fileCount);
...
void YModem::receiveStageEstablishing() {
    ...
    case CodeSoh:
        ...
        if (receiveFileOperate(StatusEstablish)) {
            abort(false); // 清除标志
            stage = StageEstablished;
            txBuffer[0] = CodeAck;
            txBuffer[1] = CodeC;
...
void YModem::receiveStageEstablished() {
    ...
    case CodeCan:
        abort(false);
        receiveFileOperate(StatusAbort);
        break;
...
void YModem::storePacket(Code _code) {
    ...
    clearCounter();
    if (receiveFileOperate(StatusTransmit, dataLength)) {
        stage = StageTransmitting;
        txBuffer[0] = CodeAck;
        txLength = 1;
        write(txBuffer, txLength);
